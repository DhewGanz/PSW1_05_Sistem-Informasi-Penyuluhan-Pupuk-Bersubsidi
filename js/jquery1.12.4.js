...
function liveSearch() {
    ...
    var elements = document.querySelectorAll('.searchable-element');
...
window.addEventListener('click', function (event) {
    ...
    dropdowns.forEach(function (dropdown) {
        if (!dropdown.contains(event.target)) {
            var openDropdown = dropdown.querySelector('.dropdown-menu');
...
function liveSearch() {
  var input, filter, elements, h2, i, txtValue;
  input = document.getElementById('search-input');
  filter = input.value.toUpperCase();
  elements = document.getElementsByClassName('searchable-element');

  for (i = 0; i < elements.length; i++) {
      h2 = elements[i].getElementsByTagName('h2')[0];
